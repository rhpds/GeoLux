...
class TestStabilityProviderContract:
    ...
    def test_threshold_update_contract(self, client):
        response = client.put("/stability/thresholds", json={"threshold": 0.75})
        assert response.status_code == 200
        assert response.json()["stability_threshold"] == 0.75

    def test_threshold_update_rejects_invalid(self, client):
        response = client.put("/stability/thresholds", json={"threshold": 2.0})
